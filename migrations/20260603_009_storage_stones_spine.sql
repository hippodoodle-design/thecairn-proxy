-- Migration 009 — storage_stones_spine (REWORKED: owner_id -> auth.users(id))
-- Wave Cairn Storage + Stones Spine (3 Jun 2026), project The Cairn (mzjvcntzcfagasxcnuye).
-- Dispatch db871533-9c38-4059-9f43-3ff9ad90e1f0 (origin) + c4f80a16 (build-to-live).
--
-- REWORK (4 Jun 2026): the original 009 referenced public.accounts(id), assuming a
-- separate accounts table from 008. The locked foundation (008, board verdict
-- e97d2527) anchors directly on auth.users(id) (profiles.id IS the auth user id);
-- there is NO public.accounts table. So all four owner_id FKs below now reference
-- auth.users(id) — the only change from the original 009. RLS still scopes every
-- row to auth.uid(), which equals owner_id.
--
...
--
-- APPLY ORDER: apply 008 (cairn_ accounts/billing foundation) BEFORE 009.
--
...
-- with the project access token (NOT the service-role PostgREST key, which cannot
-- run DDL). Idempotent (IF NOT EXISTS / CREATE OR REPLACE), non-destructive,
-- safe to re-run.
--
